...
export default function Environment() {
  ...
  const avgGas = liveZone.gas !== undefined ? liveZone.gas : 1.5;
  const avgO2 = liveZone.o2 !== undefined ? liveZone.o2 : 20.9;
  const gasSt = liveZone.status || 'SAFE';
  const gasC = gasSt === 'DANGER' ? 'bg-brand-red' : gasSt === 'WARNING' ? 'bg-orange-600' : 'bg-black';
  
  const o2St = avgO2 <= 18.5 ? 'DANGER' : avgO2 <= 19.5 ? 'WARNING' : 'SAFE';
  const o2C = o2St === 'DANGER' ? 'bg-brand-red' : o2St === 'WARNING' ? 'bg-orange-600' : 'bg-black';

  const aqi = Math.round(avgGas * 3 + (20.9 - avgO2) * 50);
  const aqiT = aqi > 200 ? 'HAZARDOUS' : aqi > 100 ? 'UNHEALTHY' : 'GOOD';
  const aqiC = aqi > 100 ? 'text-brand-red border-brand-red' : 'text-black border-black';
          ...
          <div className="flex flex-col gap-6">
            <div>
              <div className="flex justify-between font-headline text-xs mb-1">
                ...
                <span className={`font-heavy tracking-tighter ${gasSt !== 'SAFE' ? 'text-brand-red animate-pulse' : 'text-black'}`}>{avgGas.toFixed(2)} ppm ({gasSt})</span>
              </div>
              <div className="w-full h-4 border-2 border-black bg-gray-200">
                <div className={`h-full ${gasC} transition-all duration-500`} style={{ width: `${Math.min(100, (avgGas/60)*100)}%` }}></div>
              </div>
            </div>
             <div>
              <div className="flex justify-between font-headline text-xs mb-1">
                <span>O2 (OXYGEN)</span>
                <span className={`font-heavy tracking-tighter ${o2St !== 'SAFE' ? 'text-brand-red animate-pulse' : 'text-black'}`}>{avgO2.toFixed(1)}% ({o2St})</span>
              </div>
              <div className="w-full h-4 border-2 border-black bg-gray-200">
                <div className={`h-full ${o2C} transition-all duration-500`} style={{ width: `${Math.min(100, (avgO2/21)*100)}%` }}></div>
              </div>
            </div>
          </div>
        </div>
