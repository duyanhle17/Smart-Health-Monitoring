...
def main():
    ...
    if not port:
        print("❌ Không tìm thấy cổng COM nào cắm mạch ESP32!")
        print("Vui lòng cắm cáp USB và cài Driver CH340/CP210 nếu cần.")
        return

    print(f"🔌 Đang kết nối tới mạch tại cổng: {port} ...")
    try:
    ...
    # Trạng thái giả lập ToF
    current_x = 10.0
    current_y = 50.0
    direction = 1 # 1: đi tới, -1: đi lui

    # Lưu tạm giá trị để mix vào json
    last_hr = 75.0
    last_gas = 0.0 # Mạch ông bạn chưa có cảm biến MQ4, ta mock cái này thành an toàn 2.5
    ...
    while True:
        try:
            ...
            if line.startswith("{"):
                try:
                    ...
                    if data.get("type") == "hr":
                        ...
                        print(f"❤️ Nhịp tim: {last_hr} bpm")
            ...
            # Xử lý cập nhật x, y giả lập
            current_x += direction * 0.1 # Nhanh/chậm đi bộ
            if current_x > 90.0:
                direction = -1
            elif current_x < 10.0:
                direction = 1

            # Khí gas mock ngẫu nhiên loanh quanh 2.0 - 5.0
            import random
            last_gas = round(2.0 + random.random() * 3.0, 2)

            # Gửi lên API tối đa 2 lần / giây (0.5s)
            now = time.time()
